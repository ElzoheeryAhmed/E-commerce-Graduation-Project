...
namespace GraduationProject.Repository.Extensions
{
    public class SortCollection<TSource> {
        ...
        private static ISortProperty<TSource>? Parse(string element) {
            ...
            // Determining if the property being passed in matches a property name on the TSource type.
            ...
            // Constructing a generic type of SortProperty<TKey>: ISortProperty<TSource>.
        ...
        private class SortProperty<TKey> : ISortProperty<TSource> {
            ...
            public IQueryable<TSource> Apply(IQueryable<TSource> queryable) {
                ...
                if (visitor.OrderingMethodFound) {
                    queryable = Direction == ListSortDirection.Ascending ? ((IOrderedQueryable<TSource>)queryable).ThenBy(Filter) : ((IOrderedQueryable<TSource>)queryable).ThenByDescending(Filter);
                }
                
                else {
                    queryable = Direction == ListSortDirection.Ascending ? queryable.OrderBy(Filter) : queryable.OrderByDescending(Filter);
                }
            ...
            public override string ToString() {
                return Direction == ListSortDirection.Ascending ? PropertyName : $"-{PropertyName}";
            }
